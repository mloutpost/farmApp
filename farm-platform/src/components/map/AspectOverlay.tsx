...
import { generateAspectRaster } from "@/lib/dem-analysis";
...
export default function AspectOverlay() {
  ...
  const { raster, aspectVisible } = useDemStore();

  useEffect(() => {
    if (!map || !raster || !aspectVisible) {
    ...
    const { bbox } = raster;
    const { data, width: w, height: h } = generateAspectRaster(raster, MAX_DIM);
