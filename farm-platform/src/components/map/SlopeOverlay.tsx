...
import { generateSlopeRaster } from "@/lib/dem-analysis";
...
export default function SlopeOverlay() {
  ...
  const { raster, slopeVisible } = useDemStore();

  useEffect(() => {
    if (!map || !raster || !slopeVisible) {
    ...
    const { bbox } = raster;
    const { data, width: w, height: h } = generateSlopeRaster(raster, MAX_DIM);
